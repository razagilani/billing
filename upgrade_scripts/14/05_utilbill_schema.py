...
# convert utilbill_version.id into foreign key column "utilbill_id"
# TODO is there no way to add the foreign key constraint at the same time as the column is redefined, like other constraints?
...
for account, customer_id, reebill_id, start, end, service, total_charges in cur.fetchall():
    mongo_docs = db.findOne({'account': account, 'start': period_start, 'end': period_end, 'service': service})
    ...
    for doc in mongo_docs:
        ...
        cur.execute("insert into utilbill_version (customer_id, reebill_id, period_start, period_end, service, total_charges, utility) values (%s, %s, %s, %s, %s, %s, %s)",
                (reebill_id, start, end, service, total_charges, utility))
        doc['mysql_id'] = cur.lastrowid

# add constraint for uniqueness on {account, service, utility, start, end} (which we already enforce for mongo documents in ReeBillDAO)
cur.execute("alter table utilbill add constraint utilbill_version_unique unique (customer_id, service, utility, period_start, period_end)")

###############################################################################
# utilbill

# remove all data that have been moved to the utilbill_version table
cur.execute("alter table utilbill drop column rebill_id, period_start, period_end, service, total_charges")
