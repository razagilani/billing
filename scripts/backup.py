...
def restore_mongo_collection_local(collection_name, dump_file_path):
    ...
    command = MONGORESTORE_COMMAND % dict(db=config.get('usersdb', 'database'),
...
def restore(args):
    ...
    for collection in MONGO_COLLECTIONS:
        ...
        # ends with ".bson".
        bson_file_path = '/tmp/reebill_mongo_%s_%s.bson' % (collection,
                                                            datetime.utcnow())
...
if __name__ == '__main__':
    ...
    backup_parser.add_argument("--bucket", required=True, type=str,
                               help=('S3 bucket name'))
    backup_parser.add_argument("--access-key", required=True, type=str,
                               help="The S3 access key for authenticating, generated by AWS IAM")
    backup_parser.add_argument("--secret-key", required=True, type=str,
    ...
    # TODO: get rid of duplicate code here shared with the backup parser
    restore_parser.add_argument("--bucket", required=True, type=str,
                                help=('S3 bucket name'))
    restore_parser.add_argument("--access-key", required=True, type=str,
                                help="The S3 access key for authenticating, generated by AWS IAM")
    restore_parser.add_argument("--secret-key", required=True, type=str,
                                help="The S3 secret key for authenticating, generated by AWS IAM")
    # TODO: this is not shared with backup parser but is shared with restore parser
    restore_parser.add_argument("--scrub", action='store_true',
    ...
    download_parser = subparsers.add_parser('download',
                                            help=('download database dump files so they can be used '
                                                  'with "restore-local"'))
    download_parser.add_argument("--bucket", required=True, type=str,
                                 help=('S3 bucket name'))
    download_parser.add_argument("--access-key", required=True, type=str,
                                 help="The S3 access key for authenticating, generated by AWS IAM")
    download_parser.add_argument("--secret-key", required=True, type=str,
                                 help="The S3 secret key for authenticating, generated by AWS IAM")
    download_parser.add_argument('--backup-file-dir', type=str, default='/tmp',
                                 help=('Local directory containing database dump files (%s) to be '
                                       'used with restore-local' % ', '.join(get_all_file_names())))
    download_parser.set_defaults(func=download)

    # restore-local
    restore_local_parser = subparsers.add_parser('restore-local',
                                                 help=('restore databases from existing dump files in '
                                                       'local directory'))
    restore_local_parser.add_argument('--backup-file-dir', type=str, default='/tmp',
                                      help=('Local directory containing database dump files (%s) to be '
                                            'used with restore-local' % ', '.join(get_all_file_names())))
    restore_local_parser.add_argument("--scrub", action='store_true',
                                      help=('After restoring, replace parts of the data set with '
                                            'placeholder values (for development only)'))
