...
# remove test database and re-create it, so it's guaranteed to exist and has no
# tables
...
cur.execute("select table_name from information_schema.tables where table_schema = '%s' and table_type = 'BASE TABLE'" % DEV_DB);
dev_tables = [x[0] for x in cur.fetchall()]
cur.execute("select table_name from information_schema.tables where table_schema = '%s' and table_type = 'VIEW'" % DEV_DB);
