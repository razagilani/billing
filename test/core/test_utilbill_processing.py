...
from billing.processing.state import UtilBill, Session, StateDB, Customer, \
    Address
from billing.processing.utilbill_processor import UtilbillProcessor
...
class UtilbillProcessingTest(TestCaseWithSetup, testing_utils.TestCase):
    ...
    def setUp(self):
        ...
        self.rate_structure_dao = RateStructureDAO(logger=logger)
        ...
        self.utilbill_processor = UtilbillProcessor(self.rate_structure_dao,
                                         billupload, self.nexus_util)

        mongoengine.connect('test', host='localhost', port=27017,
                            alias='journal')

    # TODO: this can't go here because only ReebillProcessor can create an
    # account, but we'll need a test for just creating a utility account that
    #  is not a ReeBill customer
    ...
    #     self.utilbill_processor.create_new_account('88888', 'New Account',
    #                                            'thermal',
    ...
    #                       self.utilbill_processor.create_new_account,
    ...
    #     # utility bills yet
    #     self.utilbill_processor.create_new_account('77777', 'New Account',
    #                                            'thermal',
    #             0.6, 0.2, billing_address, service_address, '88888')
    #     self.utilbill_processor.create_new_account('66666', 'New Account',
    #                                            'thermal',
    #         0.6, 0.2, billing_address, service_address, '88888')
    #
    #     # Try rolling a reebill for a new account that has no utility bills uploaded yet
    #     self.assertRaises(NoResultFound, self.utilbill_processor.roll_reebill,
    ...
    def test_update_utilbill_metadata(self):
        ...
        doc = self.utilbill_processor.get_all_utilbills_json('99999', 0,
                                                            30)[0][0]
        ...
        # TODO: don't directly access billupload here; instead use whatever
        # method provides the URL for downloading a utility bill PDF
        bill_file_path = self.utilbill_processor.billupload.get_utilbill_file_path(utilbill)
    ...
    def test_upload_utility_bill(self):
        ...
        with open(utilbill_path) as file1:
            self.utilbill_processor.upload_utility_bill(account, 'electric',
                                             date(2012, 1, 1),
                                             date(2012, 2, 1), file1,
                                             'january.pdf',
                                             utility='pepco',
        ...
        for obj in utilbills_data:
            ...
            if state == 'Final':
                self.utilbill_processor.billupload.get_utilbill_file_path(u)
            else:
        ...
        _, new_path = self.utilbill_processor.delete_utility_bill_by_id(ids[3])
        _, count = self.utilbill_processor.get_all_utilbills_json(account, 0,
                                                                  30)
        self.assertEqual(3, count)
        self.assertTrue(os.access(new_path, os.F_OK))
        _, new_path = self.utilbill_processor.delete_utility_bill_by_id(ids[2])
        _, count = self.utilbill_processor.get_all_utilbills_json(account, 0,
                                                                  30)
        self.assertEqual(2, count)
        self.assertTrue(os.access(new_path, os.F_OK))
        _, new_path = self.utilbill_processor.delete_utility_bill_by_id(ids[1])
        _, count = self.utilbill_processor.get_all_utilbills_json(account, 0,
                                                                  30)
        self.assertEqual(1, count)
        _, new_path = self.utilbill_processor.delete_utility_bill_by_id(ids[0])
        _, count = self.utilbill_processor.get_all_utilbills_json(account, 0,
                                                                  30)
    ...
    def test_rs_prediction(self):
        ...
        # TODO this should really part of a setUp method
        session = self.session
        session.add(Customer('Customer A', acc_a, .12, .34, '',
                             'Other Utility', 'Other Rate Class',
                             Address(), Address()))
        session.add(Customer('Customer B', acc_b, .12, .34, '',
                             'Other Utility', 'Other Rate Class',
                             Address(), Address()))
        session.add(Customer('Customer B', acc_c, .12, .34, '',
                             'Other Utility', 'Other Rate Class',
                             Address(), Address()))
        # self.utilbill_processor.create_new_account(acc_a, 'Customer A',
        #                                           'thermal',
        #                                 .12, .34, billing_address,
        #                                 service_address, '100001')
        # self.utilbill_processor.create_new_account(acc_b, 'Customer B',
        #                                           'thermal',
        #                                 .12, .34, billing_address,
        #                                 service_address, '100001')
        # self.utilbill_processor.create_new_account(acc_c, 'Customer C',
        #                                           'thermal',
        #                                 .12, .34, billing_address,
        #                                 service_address, '100001')
        ...
        self.utilbill_processor.upload_utility_bill(acc_a,
        ...
        self.assertEqual([], self.utilbill_processor.get_utilbill_charges_json(
            id_a_2))
        ...
        self.assertEqual(set(['DISTRIBUTION_CHARGE', 'PGC', 'NOT_SHARED']),
                         set(r['rsi_binding'] for r in
                             self.utilbill_processor.get_utilbill_charges_json(
                                 id_a_2)))
        ...
        self.utilbill_processor.delete_charge(utilbill_id=id_a_2,
                             rsi_binding='DISTRIBUTION_CHARGE')
        self.utilbill_processor.delete_charge(utilbill_id=id_a_2,
                                           rsi_binding='PGC')
        self.utilbill_processor.delete_charge(utilbill_id=id_a_2,
    ...
    def test_compute_utility_bill(self):
        ...
        # doc = self.utilbill_processor.get_utilbill_doc(session, utilbill_data[
        # 'id'])
